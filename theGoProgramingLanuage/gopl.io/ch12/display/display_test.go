...
func Example_movie() {
	...
	// 几种类型的print map is nondeterministic.
	/*
		//!+output
...
func Test(t *testing.T) {
	...
	// 几种类型的print pointer that points to itself
	type P *P
	var p P
	p = &p
	if false {
	...
	// 几种类型的print map that contains itself
	type M map[string]M
	m := make(M)
	m[""] = m
	if false {
	...
	// 几种类型的print slice that contains itself
	type S []S
	s := make(S, 1)
	s[0] = s
	if false {
	...
	// 几种类型的print linked list that eats its own tail
	type Cycle struct {
		Value int
		Tail  *Cycle
	}
